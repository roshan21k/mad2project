...
# Send all Category details with product(nested)
@user_bp.get('/')
@jwt_required()
def user_home():
...
def user_cart(id):
    try:
        ...
        if item_exists:
            # If already in cary increment quantity by one
...
# Increment cart quantity by one 
...
# Decrement Cart quantity by one
...
# Place order for the items in cart
...
def place_order():
    try:
        ...
        # If stock is not enough send info about stock available
        ...
        # Remove item from cart and add to order details
        for item in cart_items:
...
# Give all orders placed by user
@user_bp.get('/orders')
@jwt_required()
def order():
    try:
...
# Adding review/comment on a particular product
...
# Get the request sent by me(Role Request)
